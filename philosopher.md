...
# 개념공부
## Data Racing : 경쟁 상태
 다른 곳에서 읽을 가능성이 있는 어떤 메모리 위치(공유 자원)에 쓰기 작업을 하는 것..
프로그램이나 시스템등의 실행/출력 결과가 일정하지 않고, 입력의 타이밍, 실행되는 순서나 시간 등에 영향을 받게 되는 상황. 결과가 매번 달라진다.
 이번 과제에서는 포크라는 공유 자원을 사용하면서 경쟁 상태를 방지하는 방법을, 여러 스레드 또는 프로세스(보너스)를 가지고 구현합니다. 
...
아래의 코드를 멀티 스레드로 동시실행할 경우, 뮤텍스를 걸지 않은 상태에서는 중간에 컨텍스트 스위칭이 일어나며 임의의 값이 출력될 수 있다. 그러나 아래와 같이 뮤텍스 락을 걸어둔 상태에서느
```c
void *printf_a(pthread_mutex_t *mutex)
{
	int i;
